...
namespace _05_FilterByAge
{
    class Program
    {
        ...
        static void Printer(KeyValuePair<string, int> person, string[] printerPattern)
        {
            ...
            {
                Console.WriteLine(printerPattern[0] == "name" ? $"{person.Key} - {person.Value}" : $"{person.Value} - {person.Key}");
            }
            else
            {
                Console.WriteLine(printerPattern[0] == "name" ? $"{person.Key}" : $"{person.Value}");
